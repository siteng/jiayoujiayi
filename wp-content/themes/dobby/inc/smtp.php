 ...
 * @since 1.0
 */
add_action('phpmailer_init', 'dobby_mail_smtp');
function dobby_mail_smtp( $phpmailer ) {
    if( dobby_option('mail_smtps') ){
        $mail_name = dobby_option('mail_name');
        $mail_host = dobby_option('mail_host');
        $mail_port = dobby_option('mail_port');
        $mail_username = dobby_option('mail_username');
        $mail_passwd = dobby_option('mail_passwd');
        $mail_smtpsecure = dobby_option('mail_smtpsecure');
        $phpmailer->FromName = $mail_name ? $mail_name : 'dobby'; 
        $phpmailer->Host = $mail_host ? $mail_host : 'smtp.dobby';
        $phpmailer->Port = $mail_port ? $mail_port : '994';
        $phpmailer->Username = $mail_username ? $mail_username : 'no_reply@dobby';
        $phpmailer->Password = $mail_passwd ? $mail_passwd : 'dobby';
        $phpmailer->From = $mail_username ? $mail_username : 'no_reply@dobby';
        $phpmailer->SMTPAuth = dobby_option('mail_smtpauth') ? true : false ;
        $phpmailer->SMTPSecure = $mail_smtpsecure ? $mail_smtpsecure : 'ssl';
        $phpmailer->IsSMTP();
    }
}

add_action('comment_unapproved_to_approved', 'dobby_comment_approved');
function dobby_comment_approved($comment) {
    if(is_email($comment->comment_author_email)) {
        ...
        $message = '
            <div style="background:#ececec;width: 100%;text-align:center;">
            <div style="background:#fff;width:750px;text-align:left;position:relative;margin:0 auto;font-size:14px;line-height:1.5;">
                    <div style="zoom:1;padding:25px 40px;background:#518bcb; background-image: url(https://www.sjiayi.com/wp-content/uploads/2019/01/chengzhangwangshi.jpg);text-align: center;">
 ...
                    </div>
				...
				<div style="zoom:1;padding:25px 40px;background:#518bcb;background-image: url(https://www.sjiayi.com/wp-content/uploads/2019/01/chengzhangwangshi.jpg);">
<p style="background-color=#ececec;color:#a0a0a0;line-height:18px;font-size:12px;margin:5px 0;text-align: center;">您可以点此链接 <a href="'.get_comment_link( $comment->comment_ID ).'">查看回复完整內容</a> | 感谢您的关注和支持！</p>
</div>
            </div>
        </div>';
...
add_action('comment_post', 'dobby_comment_notify');
function dobby_comment_notify($comment_id) {
    ...
    if (($parent_id != '') && ($spam_confirmed != 'spam')) {
        ...
        $message = '
            ...
            <div style="background:#fff;width:750px;position:relative;margin:0 auto;font-size:14px;line-height:1.5;">
                    <div style="zoom:1;padding:25px 40px;background:#518bcb; background-image: url(https://www.sjiayi.com/wp-content/uploads/2019/01/chengzhangwangshi.jpg);">
                        <h1 style="color:#fff; font-size:25px;line-height:30px; margin:0;text-align:center;"><a href="' . get_option('home') . '" style="text-decoration: none;color: #FFF;">' . htmlspecialchars_decode(get_option('blogname'), ENT_QUOTES) . '</a></h1>
                    </div>
                <div style="padding:35px 40px 30px;">
