	...
	<body>
		<header>
			    <nav class="navbar navbar-expand-md fixed-top navbar-dark site-header">
			        ...
			        <?php } ?>
			        <a class="navbar-brand navbar-full" href="<?php echo home_url(); ?>" title="<?php dobby_description(); ?>"><?php bloginfo('name'); ?></a>
	        		...
	        		<?php wp_nav_menu( array('theme_location' => 'header_menu', 'depth' => 2, 'container' => null, 'menu_class' => 'navbar-nav mr-auto', 'walker' => new WP_Bootstrap_Navwalker())); ?>
						...
						<?php } else { ?> 
							<a href="<?php if(dobby_option('global_login')){ echo dobby_option('global_login');} else {echo wp_login_url( 'https://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] );}?>">
