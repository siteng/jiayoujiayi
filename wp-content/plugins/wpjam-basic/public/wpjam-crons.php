...
class WPJAM_Cron{
	...
	public function __construct($hook, $args){
		$this->hook			= $hook;
		$this->jobs			= $args['jobs'];
		$this->weight		= $args['weight'] ?? false;
		$this->recurrence	= $args['recurrence'] ?? 'five_minutes';
	}
...
class WPJAM_Job{
	...
	public static function load_plugin_page($current_tab){
		wpjam_register_list_table('wpjam-jobs', [
			...
			'singular'	=> 'job',
			'model'		=> 'WPJAM_Job',
			'summary'	=> '今天已经运行 <strong>'.self::get_cron()->get_counter().'</strong> 次',
			'fixed'		=> false,
			'ajax'		=> true
		]);
	}
	...
	if(is_callable($hook)){
		...
		if(!empty($args['callback']) && is_callable($args['callback'])){
			...
			if(!wpjam_is_scheduled_event($hook)){
				$args	= wp_parse_args($args, ['recurrence'=>'', 'time'=>time(), 'args'=>[]]);
...
if(is_admin() && (!is_multisite() || !is_network_admin())){
	wpjam_add_basic_sub_page('wpjam-crons', [
		'menu_title'	=> '定时作业',
		'function'		=> 'tab',
		'tabs'			=> ['crons'=>['title'=>'定时作业',	'function'=>'list',	'load_callback'=>['WPJAM_Cron', 'load_plugin_page']]],
