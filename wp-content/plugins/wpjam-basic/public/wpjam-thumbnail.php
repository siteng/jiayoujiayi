...
class WPJAM_Thumbnail{
	...
	public function filter_term_thumbnail_url($thumbnail_url, $term){
		$term_options	= wpjam_get_term_options($term->taxonomy);

		if(isset($term_options['thumbnail'])){
	...
	public function term_single_row_replace($html){
	...
	public function filter_term_single_row_html($html){
		if(!wp_doing_ajax() || (wp_doing_ajax() && in_array($_POST['action'], ['inline-save-tax', 'add-tag']))){
			return $this->term_single_row_replace($html);
			...
			if(isset($response['data'])){
				if(is_array($response['data'])){
					$response['data']	= array_map([$this, 'term_single_row_replace'], $response['data']);
				}else{
					$response['data']	= $this->term_single_row_replace($response['data']);
				}
	...
	public function on_builtin_page_load($screen_base, $current_screen){
		if(in_array($screen_base, ['term', 'edit-tags'])){
			$term_options	= wpjam_get_term_options($current_screen->taxonomy);
			...
			if(isset($term_options['thumbnail'])){
				wpjam_register_list_table_action('set_thumbnail', [
					...
					'tb_height'		=> '400',
					'row_action'	=> false,
					'fields'		=> ['thumbnail'=>$term_options['thumbnail']],
					'callback'		=> [$this, 'set_term_thumbnail']
				]);

				add_filter('wpjam_html', [$this, 'filter_term_single_row_html']);
			}
		}
	}
...
add_action('wp_loaded', function(){
	...
	if(is_admin() && (!is_multisite() || !is_network_admin())){
		...
		add_action('wpjam_builtin_page_load',	[$instance, 'on_builtin_page_load'], 99, 2);
	}
});